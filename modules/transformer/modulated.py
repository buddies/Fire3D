...
class ModulatedTransformerCrossBlock(nn.Module):
    ...
    def _forward(self, x: torch.Tensor, mod: torch.Tensor, context: torch.Tensor, context_mask: Optional[torch.Tensor] = None, phases: Optional[torch.Tensor] = None) -> torch.Tensor:
        if self.share_mod:
            shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (self.modulation + mod).type(mod.dtype).chunk(6, dim=1)
        else:
            shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(mod).chunk(6, dim=1)
        ...
        x = x + h
        h = self.norm2(x)
        h = self.cross_attn(h, context, context_mask=context_mask)
        x = x + h
        h = self.norm3(x)
        h = h * (1 + scale_mlp.unsqueeze(1)) + shift_mlp.unsqueeze(1)
        h = self.mlp(h)
    ...
    def forward(self, x: torch.Tensor, mod: torch.Tensor, context: torch.Tensor, context_mask: Optional[torch.Tensor] = None, phases: Optional[torch.Tensor] = None) -> torch.Tensor:
        if self.use_checkpoint:
            return torch.utils.checkpoint.checkpoint(self._forward, x, mod, context, context_mask, phases, use_reentrant=False)
        else:
            return self._forward(x, mod, context, context_mask, phases)
