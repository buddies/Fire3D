...
def scaled_dot_product_attention(*args, attn_mask: Optional[torch.Tensor] = None, **kwargs):
    """
    Args:
        attn_mask: Optional [N, 1, 1, L_k] or [N, H, L_q, L_k] tensor.
                   Can be boolean (True = keep) or float (0 = keep, -inf = mask).
    """
    arg_names_dict = {
        1: ['qkv'],
        2: ['q', 'kv'],
    ...
    assert attn_mask is None or config.BACKEND == 'sdpa', "Attn mask is only supported for SDPA backend"
