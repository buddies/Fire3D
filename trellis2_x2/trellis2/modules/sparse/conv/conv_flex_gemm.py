...
def sparse_conv3d_forward(self, x: SparseTensor) -> SparseTensor:
    ...
    # A masked-implicit cache first built by a frozen/no-grad convolution omits
    # the signals used only by backward. SparseTensor.replace shares spatial
    # caches, so a later trainable convolution can encounter that forward-only
    # cache. Lazily rebuild it once with backward metadata instead of passing an
    # incomplete cache into flex_gemm autograd.
    needs_backward_cache = torch.is_grad_enabled() and (
        x.feats.requires_grad
        or self.weight.requires_grad
        or (self.bias is not None and self.bias.requires_grad)
    )
    is_masked_implicit = config.FLEX_GEMM_ALGO.startswith('masked_implicit_gemm')
    has_backward_signals = neighbor_cache is not None and all(
        hasattr(neighbor_cache, field)
        for field in ('valid_signal_i', 'valid_signal_o', 'valid_signal_seg')
    )
    if (
        neighbor_cache is not None
        and is_masked_implicit
        and needs_backward_cache
        and not has_backward_signals
    ):
        neighbor_cache = None
