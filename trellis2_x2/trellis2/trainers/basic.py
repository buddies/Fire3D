...
class BasicTrainer:
    ...
    Args:
        ...
        models_freeze (dict[str, nn.Module]): Models that are frozen (not trained) but used during training.
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        self.models = models
        self.models_freeze = models_freeze if models_freeze is not None else {}
        ...
        self.fp16_max_log_scale = fp16_max_log_scale
        ...
        if self.mix_precision_mode == 'inflat_all' and self.mix_precision_dtype == torch.float16 and self.fp16_max_log_scale is not None:
            self.log_scale = min(self.log_scale, float(self.fp16_max_log_scale))
        ...
        if self.parallel_mode == 'ddp' and self.world_size > 1 and self.ddp_init_sync:
            self.check_ddp()
    ...
    def _init_wandb(self):
        cfg = dict(self.wandb_config)
        cfg.pop('enabled', None)
        project = cfg.pop('project', 'ff_holoscene')
        name = cfg.pop('name', os.path.basename(os.path.normpath(self.output_dir)))
        run_dir = cfg.pop('dir', os.path.join(self.output_dir, 'wandb'))
        init_kwargs = {
            'project': project,
            'name': name,
            'dir': run_dir,
        }
        for key in ('entity', 'mode', 'group', 'job_type', 'tags', 'notes', 'config'):
    ...
    def __str__(self):
        ...
        if self.models_freeze:
            lines.append(f'  - Frozen Models:')
            for name, model in self.models_freeze.items():
                lines.append(f'    - {name}: {model.__class__.__name__}')
        ...
        if self.mix_precision_dtype == torch.float16 and self.mix_precision_mode in {'amp', 'inflat_all'}:
            lines.append(f'  - FP16 scale growth: {self.fp16_scale_growth}')
            if self.fp16_max_log_scale is not None:
                lines.append(f'  - FP16 max log scale: {self.fp16_max_log_scale}')
        lines.append(f'  - Parallel mode: {self.parallel_mode}')
        if self.parallel_mode == 'ddp':
            lines.append(f'  - DDP init sync: {self.ddp_init_sync}')
        return '\n'.join(lines)
    ...
    def init_models_and_more(self, **kwargs):
        ...
        """
        # Freeze models that should not be trained
        # Move to the same device as training models and set to eval mode
        for name, model in self.models_freeze.items():
    ...
    def load(self, load_dir, step=0):
        ...
        if self.world_size > 1 and self.ddp_init_sync:
            self.check_ddp()
    ...
    def run_step(self, data_list):
        ...
        if False and self.log_param_stats:
            param_norms = {}
            param_grads = {}
    ...
    def save_logs(self):
        ...
        if self.use_wandb and self.wandb_run is not None:
            self._wandb.log({key: float(value) for key, value in log_show.items()}, step=self.step)
        self.log = []
    ...
    def run(self):
        ...
        time_last_print = 0.0
        time_elapsed = 0.0
        pbar = tqdm(total=self.max_steps, initial=self.step, desc="Training", disable=not self.is_master)
        while self.step < self.max_steps:
            ...
            if self.is_master and self.step % self.i_print == 0:
                ...
                pbar.set_postfix({
                    'speed': f'{speed:.1f} steps/h',
                    'elapsed': f'{time_elapsed / 3600:.2f}h',
                })
                time_last_print = time_elapsed

            # Check ddp
            if self.parallel_mode == 'ddp' and self.world_size > 1 and self.ddp_init_sync and self.i_ddpcheck is not None and self.step % self.i_ddpcheck == 0:
                self.check_ddp()
    ...
    def profile(self, wait=1, warmup=1, active=2):
        """
        ...
        with torch.profiler.profile(
            ...
            data_list = self.load_data()
            for _ in tqdm(range(wait + warmup + active), desc="Profiling", disable=not self.is_master):
