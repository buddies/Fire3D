...
class SLat(SLatVisMixin, StandardDatasetBase):
    ...
    Args:
        ...
        preload (bool): if True, preload all instances into memory for faster access
    """
    def __init__(self,
        roots: str,
        *,
        ...
        # self.loads = [self.metadata.loc[sha256, f'latent_trellis2_shape_encoding_tokens'] for _, sha256 in self.instances]
        ...
        if self.preload:
            self._preload_all_instances()

    def filter_metadata(self, metadata):
        stats = {}
        # metadata = metadata[metadata['local_path'].notna()]
        # stats['Local path not na'] = len(metadata)
        ...
        metadata = metadata[metadata['latent_trellis2_shape_encoding']]
        stats['With latent'] = len(metadata)
        return metadata, stats

    def _preload_all_instances(self, num_workers: int = 16):
        """Preload all instances into memory for faster access using parallel threads."""
        def load_task(root_instance):
            root, instance = root_instance
            return (root, instance), self._load_instance(root, instance)

        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            futures = [executor.submit(load_task, item) for item in self.instances]
            for future in tqdm(as_completed(futures), total=len(futures), desc="Preloading instances"):
                key, value = future.result()
                self._cache[key] = value

    def _load_instance(self, root, instance):
        """Load a single instance from disk."""
        data = np.load(os.path.join(root, 'latents', self.latent_key, f'{instance}.npz'))
        coords = torch.tensor(data['coords']).int()
        feats = torch.tensor(data['feats']).to(torch.float16 if self.use_fp16 else torch.float32)
        if self.normalization is not None:
    ...
    def get_instance(self, root, instance):
        if self.preload:
            return self._cache[(root, instance)]
        return self._load_instance(root, instance)
