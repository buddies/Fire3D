...
public:
    ...
    /**
     * Run one edge-collapse step on a packed batch while respecting per-mesh targets.
     *
     * @param vertex_batch The batch id for each vertex, shape [V], int32 CUDA tensor.
     * @param face_batch The batch id for each face, shape [F], int32 CUDA tensor.
     * @param target_faces The target face count for each batch item, shape [B], int32 CUDA tensor.
     * @return Updated vertex_batch, updated face_batch, and current per-batch face counts.
     */
    std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> simplify_step_batched(
