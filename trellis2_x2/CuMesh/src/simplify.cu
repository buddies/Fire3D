...
static __global__ void count_faces_by_batch_kernel(
    const int* face_batch,
    const int F,
    const int B,
    int* face_counts
) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= F) return;
    int batch_idx = face_batch[tid];
    if (batch_idx >= 0 && batch_idx < B) {
        atomicAdd(face_counts + batch_idx, 1);
    }
}


static __global__ void get_target_reached_kernel(
    const int* face_counts,
    const int* target_faces,
    const int B,
    uint8_t* target_reached
) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= B) return;
    target_reached[tid] = face_counts[tid] <= target_faces[tid] ? 1 : 0;
}


static __global__ void collapse_edges_batched_kernel(
    float3* vertices,
    int3* faces,
    uint64_t* edges,
    const int* vertex_batch,
    const int* vert2face,
    const int* vert2face_offset,
    const float* edge_collapse_costs,
    const uint64_t* propagated_costs,
    const uint8_t * vert_is_boundary,
    const uint8_t* target_reached,
    const int V,
    const int F,
    const int E,
    const float collapse_thresh,
    ...
    int e0 = int(e >> 32);
    int e1 = int(e & 0xFFFFFFFF);
    int batch_idx = vertex_batch[e0];
    if (target_reached[batch_idx]) return;
...
static __global__ void compress_vertex_batch_kernel(
    const int* vertices_map,
    const int* old_vertex_batch,
    const int V,
    int* new_vertex_batch
) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= V) return;
    int new_id = vertices_map[tid];
    int is_kept = vertices_map[tid + 1] == new_id + 1;
    if (is_kept) {
        new_vertex_batch[new_id] = old_vertex_batch[tid];
    }
}
...
static __global__ void compress_face_batch_kernel(
    const int* faces_map,
    const int* old_face_batch,
    const int F,
    int* new_face_batch
) {
    const int tid = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    int is_kept = faces_map[tid + 1] == new_id + 1;
    if (is_kept) {
        new_face_batch[new_id] = old_face_batch[tid];
    }
}
...
torch::Tensor count_faces_by_batch(
    const torch::Tensor& face_batch,
    const int batch_size
) {
    auto face_counts = torch::zeros({batch_size}, face_batch.options());
    int F = face_batch.size(0);
    if (F > 0) {
        count_faces_by_batch_kernel<<<(F+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(
            face_batch.data_ptr<int>(),
            F,
            batch_size,
            face_counts.data_ptr<int>()
        );
        CUDA_CHECK(cudaGetLastError());
    }
    return face_counts;
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> collapse_edges_batched(
    CuMesh& ctx,
    const torch::Tensor& vertex_batch,
    const torch::Tensor& face_batch,
    const torch::Tensor& target_faces,
    float collapse_thresh
) {
    size_t V = ctx.vertices.size;
    size_t F = ctx.faces.size;
    size_t E = ctx.edges.size;
    int B = target_faces.size(0);

    torch::Tensor face_counts = count_faces_by_batch(face_batch, B);
    auto target_reached = torch::empty({B}, face_batch.options().dtype(torch::kUInt8));
    get_target_reached_kernel<<<(B+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(
        face_counts.data_ptr<int>(),
        target_faces.data_ptr<int>(),
        B,
        target_reached.data_ptr<uint8_t>()
    );
    CUDA_CHECK(cudaGetLastError());
    ...
    ctx.vertices_map.fill(1);
    ctx.faces_map.fill(1);
    collapse_edges_batched_kernel<<<(E+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(
        ctx.vertices.ptr,
        ctx.faces.ptr,
        ctx.edges.ptr,
        vertex_batch.data_ptr<int>(),
        ctx.vert2face.ptr,
        ctx.vert2face_offset.ptr,
        ctx.edge_collapse_costs.ptr,
        ctx.propagated_costs.ptr,
    ...
    CUDA_CHECK(cudaGetLastError());
    auto new_vertex_batch = torch::empty({new_num_vertices}, vertex_batch.options());
    compress_vertex_batch_kernel<<<(V+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(
        ctx.vertices_map.ptr,
        vertex_batch.data_ptr<int>(),
        V,
        new_vertex_batch.data_ptr<int>()
    );
    CUDA_CHECK(cudaGetLastError());
    swap_buffers(ctx.temp_storage, ctx.vertices);
    ...
    CUDA_CHECK(cudaGetLastError());
    auto new_face_batch = torch::empty({new_num_faces}, face_batch.options());
    compress_face_batch_kernel<<<(F+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE>>>(
        ctx.faces_map.ptr,
        face_batch.data_ptr<int>(),
        F,
        new_face_batch.data_ptr<int>()
    );
    CUDA_CHECK(cudaGetLastError());
    swap_buffers(ctx.temp_storage, ctx.faces);

    face_counts = count_faces_by_batch(new_face_batch, B);
    return std::make_tuple(new_vertex_batch, new_face_batch, face_counts);
}
...
std::tuple<int, int> CuMesh::simplify_step(float lambda_edge_length, float lambda_skinny, float threshold, bool timing) {
    ...
    this->edge2face_cnt.free();
    this->boundaries.free();
    if (timing) {
        CUDA_CHECK(cudaDeviceSynchronize());
        end = std::chrono::high_resolution_clock::now();
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> CuMesh::simplify_step_batched(
    const torch::Tensor& vertex_batch,
    const torch::Tensor& face_batch,
    const torch::Tensor& target_faces,
    float lambda_edge_length,
    ...
    TORCH_CHECK(vertex_batch.is_cuda() && face_batch.is_cuda() && target_faces.is_cuda(),
        "vertex_batch, face_batch, and target_faces must be CUDA tensors");
    TORCH_CHECK(vertex_batch.scalar_type() == torch::kInt32 &&
                face_batch.scalar_type() == torch::kInt32 &&
                target_faces.scalar_type() == torch::kInt32,
        "vertex_batch, face_batch, and target_faces must be int32 tensors");
    TORCH_CHECK(vertex_batch.is_contiguous() && face_batch.is_contiguous() && target_faces.is_contiguous(),
        "vertex_batch, face_batch, and target_faces must be contiguous");
    TORCH_CHECK(vertex_batch.dim() == 1 && face_batch.dim() == 1 && target_faces.dim() == 1,
        "vertex_batch, face_batch, and target_faces must be 1D tensors");
    TORCH_CHECK(vertex_batch.size(0) == static_cast<int64_t>(this->vertices.size),
        "vertex_batch length must match the current number of vertices");
    TORCH_CHECK(face_batch.size(0) == static_cast<int64_t>(this->faces.size),
        "face_batch length must match the current number of faces");
    ...
    if (timing) start = std::chrono::high_resolution_clock::now();
    auto result = collapse_edges_batched(*this, vertex_batch, face_batch, target_faces, threshold);
    if (timing) {
        CUDA_CHECK(cudaDeviceSynchronize());
        end = std::chrono::high_resolution_clock::now();
        std::cout << "collapse_edges_batched: " << std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() << " us" << std::endl;
    }
