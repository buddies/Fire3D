...
template<typename T>
static __global__ void get_vertex_num_batched(
    const size_t N,
    const size_t M,
    const int W,
    const int H,
    ...
    for (int i = 0; i <= 1; i++) {
        ...
        for (int j = 0; j <= 1; j++) {
            ...
            for (int k = 0; k <= 1; k++) {
                ...
                flat_idx = (size_t)b * W * H * D + (size_t)(xx * H + yy) * D + zz;
                key = static_cast<T>(flat_idx);
                if (linear_probing_lookup(hashmap_keys, hashmap_vals, key, N) == std::numeric_limits<uint32_t>::max()) {
                    num++;
                }
...
template<typename T>
static __global__ void set_vertex_batched(
    const size_t N,
    const size_t M,
    const int W,
    const int H,
    ...
    int ptr_start = vertices_offset[thread_id];
    reinterpret_cast<int4*>(vertices)[ptr_start] = make_int4(b, x, y, z);
    ptr_start++;
    ...
    for (int i = 0; i <= 1; i++) {
        ...
        for (int j = 0; j <= 1; j++) {
            ...
            for (int k = 0; k <= 1; k++) {
                ...
                int zz = z + k;
                if (xx >= W || yy >= H || zz >= D) {
                    reinterpret_cast<int4*>(vertices)[ptr_start] = make_int4(b, xx, yy, zz);
                    ptr_start++;
                    continue;
                }
                flat_idx = (size_t)b * W * H * D + (size_t)(xx * H + yy) * D + zz;
...
torch::Tensor cumesh::get_sparse_voxel_grid_active_vertices_batched(
    torch::Tensor& hashmap_keys,
    torch::Tensor& hashmap_vals,
    const torch::Tensor& coords,
    const int W,
    ...
    size_t M = coords.size(0);
    if (M == 0) {
        return torch::empty({0, 4}, torch::dtype(torch::kInt32).device(hashmap_keys.device()));
    }
    ...
    CUDA_CHECK(cudaMalloc(&num_vertices, (M + 1) * sizeof(int)));
    if (hashmap_keys.dtype() == torch::kUInt32) {
        get_vertex_num_batched<<<(M + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE>>>(
            N,
            M,
            W,
            H,
    ...
    auto vertices = torch::empty({total_vertices, 4}, torch::dtype(torch::kInt32).device(hashmap_keys.device()));
    if (hashmap_keys.dtype() == torch::kUInt32) {
        set_vertex_batched<<<(M + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE>>>(
            N,
            M,
            W,
            H,
