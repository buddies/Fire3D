...
template<typename T>
__device__ __forceinline__ float get_vertex_val_batched(
    const T* __restrict__ hashmap_keys,
    const uint32_t* __restrict__ hashmap_vals,
    const float* __restrict__ udf,
    const size_t N_vert,
...
template<typename T>
static __global__ void simple_dual_contour_batched_kernel(
    const size_t N_vert,
    const size_t M,
    const int W,
    const int H,
    ...
    for (int u = 0; u <= 1; ++u) {
        #pragma unroll
        for (int v = 0; v <= 1; ++v) {
            float val1 = get_vertex_val_batched(hashmap_keys, hashmap_vals, udf, N_vert, b, vx, vy + u, vz + v, W, H, D);
            float val2 = get_vertex_val_batched(hashmap_keys, hashmap_vals, udf, N_vert, b, vx + 1, vy + u, vz + v, W, H, D);
            ...
            if (u == 1 && v == 1) {
                if (val1 < 0 && val2 >= 0) out_intersected[thread_id * 3 + 0] = 1;
                else if (val1 >= 0 && val2 < 0) out_intersected[thread_id * 3 + 0] = -1;
                else out_intersected[thread_id * 3 + 0] = 0;
            }
        }
    }
    ...
    for (int u = 0; u <= 1; ++u) {
        #pragma unroll
        for (int v = 0; v <= 1; ++v) {
            float val1 = get_vertex_val_batched(hashmap_keys, hashmap_vals, udf, N_vert, b, vx + u, vy, vz + v, W, H, D);
            float val2 = get_vertex_val_batched(hashmap_keys, hashmap_vals, udf, N_vert, b, vx + u, vy + 1, vz + v, W, H, D);
            ...
            if (u == 1 && v == 1) {
                if (val1 < 0 && val2 >= 0) out_intersected[thread_id * 3 + 1] = 1;
                else if (val1 >= 0 && val2 < 0) out_intersected[thread_id * 3 + 1] = -1;
                else out_intersected[thread_id * 3 + 1] = 0;
            }
        }
    }
    ...
    for (int u = 0; u <= 1; ++u) {
        #pragma unroll
        for (int v = 0; v <= 1; ++v) {
            float val1 = get_vertex_val_batched(hashmap_keys, hashmap_vals, udf, N_vert, b, vx + u, vy + v, vz, W, H, D);
            float val2 = get_vertex_val_batched(hashmap_keys, hashmap_vals, udf, N_vert, b, vx + u, vy + v, vz + 1, W, H, D);
            ...
            if (u == 1 && v == 1) {
                if (val1 < 0 && val2 >= 0) out_intersected[thread_id * 3 + 2] = 1;
                else if (val1 >= 0 && val2 < 0) out_intersected[thread_id * 3 + 2] = -1;
                else out_intersected[thread_id * 3 + 2] = 0;
            }
        }
    }
...
std::tuple<torch::Tensor, torch::Tensor> cumesh::simple_dual_contour_batched(
    const torch::Tensor& hashmap_keys,
    const torch::Tensor& hashmap_vals,
    const torch::Tensor& coords,
    const torch::Tensor& udf,
    ...
    if (hashmap_keys.dtype() == torch::kUInt32) {
        simple_dual_contour_batched_kernel<<<blocks, threads>>>(
            N_vert,
            M,
            W, H, D,
            hashmap_keys.data_ptr<uint32_t>(),
