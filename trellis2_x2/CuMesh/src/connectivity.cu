...
void CuMesh::get_edges() {
    ...
    swap_buffers(this->edges, this->temp_storage);
    ...
    CUDA_CHECK(cub::DeviceRunLengthEncode::Encode(
        nullptr, temp_storage_bytes,
        this->edges.ptr, reinterpret_cast<uint64_t*>(this->temp_storage.ptr), this->edge2face_cnt.ptr, num_edges,
        F * 3
    ));
    this->cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceRunLengthEncode::Encode(
    ...
    int new_num_edges;
    CUDA_CHECK(cudaMemcpy(&new_num_edges, num_edges, sizeof(int), cudaMemcpyDeviceToHost));
    this->temp_storage.size = new_num_edges * sizeof(uint64_t);
    swap_buffers(this->edges, this->temp_storage);
    this->edges.size = new_num_edges;
    this->edge2face_cnt.size = new_num_edges;
...
void CuMesh::get_boundary_info() {
    ...
    cub::CountingInputIterator<int> cu_edge_idx(0);
