...
def _as_obb_dict(bbox: dict, largest_scale: bool = False) -> dict:
    ...
    if "translation" in bbox:
        translation = np.asarray(bbox["translation"], dtype=np.float64).reshape(3)
    elif "translate" in bbox:
        translation = np.asarray(bbox["translate"], dtype=np.float64).reshape(3)
    else:
        raise KeyError("OBB must contain either 'translation' or 'translate'")
    if "rotation" in bbox:
...
def normalize_obb(bbox: dict, largest_scale: bool = False) -> dict:
    """Public OBB normalization helper shared by standalone evaluators."""

    return _as_obb_dict(bbox, largest_scale=largest_scale)


def obb_iou(bbox_a: dict, bbox_b: dict) -> float:
    """Public wrapper for the repository's established oriented-box IoU."""

    return _obb_iou(bbox_a, bbox_b)
...
def _bbox_instance_id(bbox: dict, fallback=None):
    for key in ("index", "instance", "instance_id", "label", "id", "object_id"):
...
def reassign_pred_labels(
    pred_labels: np.ndarray,
    pairs,
    preserve_background_label: bool = False,
):
    pred_labels = np.asarray(pred_labels).reshape(-1)
    reassigned = np.full(pred_labels.shape, -1, dtype=np.int32)
    if preserve_background_label:
        reassigned[pred_labels == 0] = 0

    for pair_idx, (pred_bbox, gt_bbox, _) in enumerate(pairs):
        pred_id = _bbox_instance_id(pred_bbox, fallback=pair_idx)
        gt_id = _bbox_instance_id(gt_bbox, fallback=pair_idx)
        if pred_id is None or gt_id is None:
            continue
        reassigned[pred_labels == pred_id] = int(gt_id)
...
def calculate_mAP_and_mIoU(
    pred_bboxes, gt_bboxes,
    pred_labels, gt_labels,
    preserve_background_label: bool = False,
):
    mAP, pairs = calculate_mAP(pred_bboxes, gt_bboxes)
    reassigned_pred_labels = reassign_pred_labels(
        pred_labels,
        pairs,
        preserve_background_label=preserve_background_label,
    )
    gt_labels = np.asarray(gt_labels).reshape(-1)
    if reassigned_pred_labels.shape != gt_labels.shape:
        raise ValueError(
            f"pred_labels and gt_labels must have the same shape after reassignment, got "
            f"{reassigned_pred_labels.shape} and {gt_labels.shape}"
        )

    nonnegative_labels = np.concatenate(
        [
            gt_labels[gt_labels >= 0],
            reassigned_pred_labels[reassigned_pred_labels >= 0],
        ]
    )
    if nonnegative_labels.size == 0:
        return mAP, float("nan"), []

    num_classes = int(nonnegative_labels.max()) + 1
    mIoU, iou_per_class = calculate_mIoU(reassigned_pred_labels, gt_labels, num_classes)
    return mAP, mIoU, iou_per_class
