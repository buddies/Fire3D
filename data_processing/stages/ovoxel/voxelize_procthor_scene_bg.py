#!/usr/bin/env python3
"""Voxelize normalized ProcTHOR room background geometry.

Room background meshes come from the released ProcTHOR adapter via
export_rooms_layout_from_scene(...)[room_id]["bg"]["canonical_mesh"]. Outputs
are written under <ProcTHOR>/ovoxels/scene_bg by default.
"""
...
def safe_room_name(scene_name: str, room_id: str) -> str:
    raw = f"{scene_name.replace('/', '__')}__room_{room_id}"
...
def process_bg_mesh(scene_name: str, room_id: str, mesh, args: argparse.Namespace) -> dict[str, object]:
    safe = safe_room_name(scene_name, room_id)
    ...
    try:
        if args.skip_existing:
            ...
            # dual_grid_from_dump/pbr_from_dump intentionally skip valid outputs.
            # Remove stale bg VXZs here so --overwrite really regenerates them.
            if args.mode in {"shape", "both"} and shape_path.exists():
                shape_path.unlink()
            if args.mode in {"pbr", "both"} and pbr_path.exists():
                pbr_path.unlink()
...
def process_scene(scene_name: str, args: argparse.Namespace) -> list[dict[str, object]]:
    procthor_utils = configure_procthor_import(args)
    _room_regions, room_geoms_dict = procthor_utils.export_rooms_layout_from_scene(scene_name)
    room_ids = sorted(room_geoms_dict)
    ...
    for room_id in room_ids:
        bg_mesh = room_geoms_dict[room_id]["bg"]["canonical_mesh"]
        records.append(process_bg_mesh(scene_name, str(room_id), bg_mesh, args))
    return records
...
def parse_args() -> argparse.Namespace:
    ...
    if args.output_dir is None:
        args.output_dir = args.procthor_root / "ovoxels" / "scene_bg"
    if args.overwrite:
