#!/usr/bin/env python3
"""Encode right-angle-rotated PBR ovoxels into original TRELLIS2 PBR latents.

This mirrors shape_enc_rot_aug_seq.py but targets PBR ovoxels and the original
TRELLIS2 texture encoder:

    microsoft/TRELLIS.2-4B/ckpts/tex_enc_next_dc_f16c32_fp16

Inputs:

    <postprocess-root>/pbr_ovoxels/<dataset>/*.vxz

Outputs:

    <output-root>/<dataset>/metadata.csv
    <output-root>/<dataset>/latents/trellis2_pbr_encoding/<asset>__rot090.npz
    <output-root>/<dataset>/skips/trellis2_pbr_encoding/<asset>__rot090.skip.json
"""
...
DEFAULT_OUTPUT_ROOT = Path(
    os.environ.get("FIRE3D_PBR_LATENT_ROOT", OBJECT_ROOT / "pbr_latents_rotated")
)
...
DATASET_NAMES = {
    ...
    "ObjaverseXL_sketchfab": "ObjaverseXL_sketchfab",
    "internscenes_partnet_smoke": "internscenes_partnet_smoke",
    "objathor": "objathor",
    "Objathor": "objathor",
    "OBJATHOR": "objathor",
    "procthor": "procthor",
...
def make_pbr_encoder_input(coords: torch.Tensor, attr: dict[str, torch.Tensor]) -> sp.SparseTensor:
    missing = [key for key in PBR_ATTRS if key not in attr]
    if missing:
        raise KeyError(f"missing PBR attrs: {missing}")
    feats = torch.cat([attr[key] for key in PBR_ATTRS], dim=-1) / 255.0 * 2.0 - 1.0
...
def load_voxel_item(voxel_path: Path, asset_id: str) -> tuple[str, torch.Tensor, dict[str, torch.Tensor]] | None:
    try:
        ...
        if coords.shape[0] == 0:
            raise ValueError("empty pbr ovoxel")
        missing = [key for key in PBR_ATTRS if key not in attr]
        if missing:
            raise ValueError(f"missing PBR attrs: {missing}; found {sorted(attr.keys())}")
        return asset_id, coords.to(torch.int32), {key: attr[key] for key in PBR_ATTRS}
...
def save_latent(save_path: Path, feats_np: np.ndarray, coords_np: np.ndarray) -> None:
    ...
    with tmp_path.open("wb") as f:
        np.savez_compressed(f, feats=feats_np, coords=coords_np)
...
def read_metadata_part(path: Path) -> list[dict[str, object]]:
...
def merge_metadata(output_dir: Path) -> Path:
    ...
    if part_dir.exists():
        for part_path in sorted(part_dir.glob("metadata_rank*.csv")):
            records.extend(read_metadata_part(part_path))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--batch-size", type=int, default=32, help="Number of rotated variants per encoder batch")
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for TRELLIS PBR encoding")

    voxel_dir = args.postprocess_root / "pbr_ovoxels" / dataset_name
    ...
    loader_pool = ThreadPoolExecutor(max_workers=args.num_loader_workers, thread_name_prefix="pbr_rot_aug_loader")
    saver_pool = ThreadPoolExecutor(max_workers=args.num_saver_workers, thread_name_prefix="pbr_rot_aug_saver")

    pending = deque()
    save_futures = deque()
    next_idx = 0
    records: list[dict[str, object]] = []
    variant_tensors: list[sp.SparseTensor] = []
    ...
    def encode_variant_batch(
        tensor_items: list[sp.SparseTensor],
        names: list[str],
        source_ids: list[str],
        rotations: list[int],
    ) -> None:
        if not names:
            return
        try:
            batch = sparse_cat(tensor_items, dim=0)
            z = encoder(batch.cuda())
            if not torch.isfinite(z.feats).all():
                if len(names) > 1:
                    mid = len(names) // 2
                    print(f"[Split] Non-finite latent batch of {len(names)} variants", flush=True)
                    encode_variant_batch(tensor_items[:mid], names[:mid], source_ids[:mid], rotations[:mid])
                    encode_variant_batch(tensor_items[mid:], names[mid:], source_ids[mid:], rotations[mid:])
                else:
    ...
    def flush_batch() -> None:
        nonlocal variant_tensors, variant_names, variant_source_ids, variant_rotations
        if not variant_names:
            return
        tensor_items = variant_tensors
        names = variant_names
        ...
        variant_rotations = []
        try:
            encode_variant_batch(tensor_items, names, source_ids, rotations)
        finally:
            torch.cuda.empty_cache()

    try:
        submit_next()
        pbar = tqdm(total=len(asset_ids), desc=f"Encoding rotated {dataset_name} PBR latents")
        while pending:
            ...
            for degrees in args.rotations:
                ...
                if args.overwrite and skip_path.exists():
                    skip_path.unlink()
                rotated_coords = rotate_coords(coords, degrees, args.grid_size).to(torch.int32)
                tensor = make_pbr_encoder_input(rotated_coords, attr)
                if not is_valid_sparse_tensor(tensor):
