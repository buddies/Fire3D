...
from utils import export_rooms, export_rooms_with_canonical_meshes
from utils import save_glb, EXPORT_DIR
...
class CameraPlannerEnv:
    def __init__(self, room_bounds, all_meshes, interest_meshes, room_region):
        ...
        room_region: xy polygon of the room region
        """
    ...
    def is_valid_location(self, point, min_dist=0.1):
        ...
        # A'. check whether point x y is inside the room region polygon
...
def get_room_meshes_new_dataset(room_geoms):
    ...
    """
    mesh_info_dict = {}
    room_object_names = [mesh_name for mesh_name in sorted(list(room_geoms.keys())) if mesh_name not in ["bg"]]
    room_mesh_names = ["bg"] + room_object_names
    for mesh_idx, mesh_name in enumerate(room_object_names):
        print(mesh_idx, mesh_name)
        mesh_info = room_geoms[mesh_name]
        mesh_info_dict[f"{mesh_idx:04d}"] = {"mesh": mesh_info["transformed_mesh"].copy()}

    mesh_info_dict["bg"] = {"mesh": room_geoms["bg"]["transformed_mesh"].copy()}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--engine", type=str, default="BLENDER_EEVEE_NEXT", choices=["CYCLES", "BLENDER_EEVEE"], help="Render engine")
    ...
    try:
        ...
        room_regions, room_geoms_dict = export_rooms_with_canonical_meshes(scene_name)

        scene_save_name = scene_name.replace("/", "_")

        for room_idx, room_geoms in room_geoms_dict.items():
            output_dir = os.path.join(args.output_dir, f"{scene_save_name}_room_{room_idx}")
