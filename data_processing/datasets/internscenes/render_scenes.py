...
try:
    ...
    SceneComposer = None
    AssetMeshLoader = None
    _HAS_COMPOSE_SCENES = False
...
class CameraPlannerEnv:
    def __init__(self, room_bounds, all_meshes, interest_meshes=None, floor_mesh=None, ceiling_mesh=None):
        """
        ...
        floor_mesh, ceiling_mesh: optional, for ray-based inside-room test (new dataset)
        """
        self.bounds = np.array(room_bounds)
        self.floor_mesh = trimesh.util.concatenate(floor_mesh.dump())
        self.ceiling_mesh = trimesh.util.concatenate(ceiling_mesh.dump())
        self._floor_intersector = None
        self._ceiling_intersector = None
        if self.floor_mesh is not None and len(self.floor_mesh.faces) > 0:
            try:
                from trimesh.ray.ray_pyembree import RayMeshIntersector
                self._floor_intersector = RayMeshIntersector(self.floor_mesh)
            except ImportError:
                from trimesh.ray.ray_triangle import RayMeshIntersector
                self._floor_intersector = RayMeshIntersector(self.floor_mesh)
        if self.ceiling_mesh is not None and len(self.ceiling_mesh.faces) > 0:
            try:
                from trimesh.ray.ray_pyembree import RayMeshIntersector
                self._ceiling_intersector = RayMeshIntersector(self.ceiling_mesh)
            except ImportError:
                from trimesh.ray.ray_triangle import RayMeshIntersector
                self._ceiling_intersector = RayMeshIntersector(self.ceiling_mesh)
    ...
    def is_valid_location(self, point, min_dist=0.1):
        ...
        # A'. Ray-based inside test for non-rectangular rooms (new dataset)
        if self._floor_intersector is not None:
            # Ray from point downward must hit floor (point is above floor)
            ray_origin = np.array([point], dtype=np.float64)
            ray_direction = np.array([[0, 0, -1]], dtype=np.float64)
            hit_locs, _, _ = self._floor_intersector.intersects_location(ray_origins=ray_origin, ray_directions=ray_direction)
            if len(hit_locs) == 0:
                return False
            # Optionally: point should be below ceiling (ray up hits ceiling)
        if self._ceiling_intersector is not None:
            ray_origin = np.array([point], dtype=np.float64)
            ray_direction = np.array([[0, 0, 1]], dtype=np.float64)
            hit_locs, _, _ = self._ceiling_intersector.intersects_location(ray_origins=ray_origin, ray_directions=ray_direction)
            if len(hit_locs) == 0:
                return False
...
def generate_camera_trajectory(
    ...
    if env is None:
        ...
        env = CameraPlannerEnv(room_bounds, all_meshes, floor_mesh=floor_mesh, ceiling_mesh=ceiling_mesh)
...
def _compute_room_bounds_from_floor_ceiling(floor_mesh, ceiling_mesh, wall_mesh=None, margin=0.05):
    """
    ...
    """
    all_verts = []
    if floor_mesh is not None and len(floor_mesh.vertices) > 0:
        all_verts.append(floor_mesh.vertices)
    if ceiling_mesh is not None and len(ceiling_mesh.vertices) > 0:
        all_verts.append(ceiling_mesh.vertices)
    if wall_mesh is not None and len(wall_mesh.vertices) > 0:
        all_verts.append(wall_mesh.vertices)
    if not all_verts:
...
def get_room_meshes_new_dataset(scene_name, scene_info_dir, use_texture=False):
    """
    ...
    """
    if not _HAS_COMPOSE_SCENES:
        raise ImportError("compose_scenes is required for new dataset loading.")
    composer = SceneComposer()
    composer.scene_info_dir = scene_info_dir
    mesh_info_dict = composer.get_flat_mesh_dict(
        scene_name,
    ...
    floor_mesh = mesh_info_dict.get("floor", {}).get("mesh")
    ceiling_mesh = mesh_info_dict.get("ceiling", {}).get("mesh")
    wall_mesh = mesh_info_dict.get("wall", {}).get("mesh")
    if floor_mesh is None:
        floor_mesh = trimesh.Trimesh()
    if ceiling_mesh is None:
        ceiling_mesh = trimesh.Trimesh()
    bounds = _compute_room_bounds_from_floor_ceiling(
        trimesh.util.concatenate(floor_mesh.dump()),
        trimesh.util.concatenate(ceiling_mesh.dump()),
        trimesh.util.concatenate(wall_mesh.dump())
    )
    # Object keys in mesh_info_dict are "0", "1", "2", ... (from compose_scene_from_instance_infos node names)
    object_ids = sorted(
        [k for k in mesh_info_dict if k not in ("floor", "wall", "ceiling") and str(k).isdigit()],
        key=int,
    )
    all_meshes = []
    interest_meshes = []
    for key in ["floor", "wall", "ceiling"] + object_ids:
    ...
    return all_meshes, interest_meshes, mesh_info_dict, bounds, object_ids, room_size_dict, floor_mesh, ceiling_mesh
        ...
        try:
            ...
            for obj in new_objects:
                if obj.type == 'MESH':
                    ...
                    obj.name = f"{mesh_id}_{object_idx_mesh_id:04d}"
...
def setup_blender_scene_for_rendering(room, mesh_info_dict, output_dir, resolution, render_depth=True, room_size_dict=None):
    ...
    if args.engine == 'CYCLES':
        ...
        for attr in ('denoiser', 'preview_denoiser'):
            if hasattr(scene.cycles, attr):
                try:
                    ...
                    setattr(scene.cycles, 'denoiser', 'OPENIMAGEDENOISE')
        ...
        # Clamp bright pixels to stop "fireflies"
        scene.cycles.sample_clamp_direct = 1.0
        scene.cycles.sample_clamp_indirect = 1.0

        # Disable Caustics (often causes noise on messy glass/shiny meshes)
        scene.cycles.caustics_reflective = False
        scene.cycles.caustics_refractive = False

        # Turn off Blue Noise if it's causing pattern artifacts
...
if __name__ == "__main__":
    ...
    parser.add_argument("scene_name", type=str, help="Scene name (e.g. arkitscenes/Training/40753686); uses Layout_info/<scene_name>")
    parser.add_argument("--scene_info_dir", type=str, default=None, help="Root dir for layout/structure (default: Layout_info from compose_scenes)")
    ...
    args.output_dir = os.path.join(args.output_dir, args.scene_name.replace("/", "_"))
    os.makedirs(args.output_dir, exist_ok=True)

    try:
        if not _HAS_COMPOSE_SCENES:
            print("Error: compose_scenes is required. Install or add to path.")
            sys.exit(1)
        scene_name = args.scene_name
        scene_info_dir = args.scene_info_dir
        if scene_info_dir is None:
            from compose_scenes import SCENE_INFO_DIR
            scene_info_dir = SCENE_INFO_DIR
        print(f"Loading scene: scene_name={scene_name}, scene_info_dir={scene_info_dir}")
        all_meshes, interest_meshes, mesh_info_dict, bounds, object_ids, room_size_dict, floor_mesh, ceiling_mesh = get_room_meshes_new_dataset(
            scene_name, scene_info_dir, use_texture=True
        )
        # debug
        # concat all_meshes into a single trimesh
        # all_meshes_concat = trimesh.util.concatenate(all_meshes)
        # save to output_dir
        # all_meshes_concat.export(os.path.join(args.output_dir, "all_meshes_concat.ply"))
        # print(f"saved all_meshes_concat to {os.path.abspath(os.path.join(args.output_dir, 'all_meshes_concat.ply'))}")
        # assert False
        # save each mesh to output_dir
        # for mesh_name, mesh in mesh_info_dict.items():
        #     mesh["mesh"].export(os.path.join(args.output_dir, f"{mesh_name}.glb"))
        room = _make_room_like(object_ids)
        scene_id = scene_name.replace("/", "_")
        print("Building environment (with floor/ceiling ray test)...")
        env = CameraPlannerEnv(bounds, all_meshes, interest_meshes, floor_mesh=floor_mesh, ceiling_mesh=ceiling_mesh)
