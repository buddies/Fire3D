...
DATASET_ROOT = os.environ.get(
    ...
    os.path.join(REPO_ROOT, "data", "training_scenes", "Imaginarium"),
)
SCENE_DIR = os.path.join(DATASET_ROOT, "scenes")
TRANSFORMS_DIR = os.path.join(DATASET_ROOT, "transforms")
...
def get_room_meshes_new_dataset(scene_name):
    ...
    """
    scene_save_dir = os.path.join(DATASET_ROOT, "scenes", scene_name)
    transforms_save_path = os.path.join(DATASET_ROOT, "transforms", f"{scene_name}.pkl")
        ...
        for node in node_tree.nodes:
            if node.type == 'BSDF_PRINCIPLED':
                try:
                    ...
                    print(f"Roughness set to {node.inputs['Roughness'].default_value}")
                except KeyError:
                    pass

                # Emission: neutral defaults (no glow) — matches fresh Principled BSDF
                try:
                    node.inputs["Emission Color"].default_value = (0.0, 0.0, 0.0, 1.0)
                    print(f"Emission Color set to {node.inputs['Emission Color'].default_value}")
                except KeyError:
                    pass

                try:
                    node.inputs["Emission Strength"].default_value = 0.0
                    print(f"Emission Strength set to {node.inputs['Emission Strength'].default_value}")
                except KeyError:
                    pass

                try:
                    node.inputs["Alpha"].default_value = 1.0
                    print(f"Alpha set to {node.inputs['Alpha'].default_value}")
                except KeyError:
                ...
                for spec_name in ("Specular", "Specular IOR Level"):
                    try:
                        ...
                        print(f"{spec_name} set to {node.inputs[spec_name].default_value}")
                        break
                    except KeyError:
                        pass

                # Drop material normal maps — use geometric normals only (Blender 4.x Principled BSDF)
                for normal_name in ("Normal", "Clearcoat Normal"):
                    try:
                        sock = node.inputs[normal_name]
...
if __name__ == "__main__":
    ...
    parser.add_argument("scene_name", type=str, help="Scene name (e.g. computer_room_01);")
