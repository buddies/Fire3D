...
from utils import get_room_geoms, room_polygon_to_xy_array
...
class CameraPlannerEnv:
    def __init__(self, room_bounds, all_meshes, interest_meshes, room_region):
        """
        ...
        room_region: (N, 2) xy vertices of the room footprint (closed ring), or None
        """
    ...
    def is_valid_location(self, point, min_dist=0.1):
        ...
        if not (np.all(point >= self.bounds[:3]) and np.all(point <= self.bounds[3:])):
            # print(f"point is outside the bounds")
            return False

        # A'. check whether point x y is inside the room region polygon (matplotlib Path)
        if self.room_region is not None and len(self.room_region) >= 3:
            xy = (float(point[0]), float(point[1]))
            if not Path(np.asarray(self.room_region)).contains_point(xy, radius=-1e-6):
                # print(f"point is outside the room region")
                return False
        ...
        if dist < min_dist:
            # print(f"point is too close to obstacles")
            return False
...
def get_room_meshes_new_dataset(room_geoms):
    """
    ...
    for mesh_idx, mesh_name in enumerate(all_room_objects):
        mesh_info = room_geoms[mesh_name]
        mesh_info_dict[f"{mesh_idx:04d}"] = {"mesh": mesh_info["mesh"].copy()}

    mesh_info_dict["bg"] = {"mesh": room_geoms["bg"]["mesh"].copy()}

    assert "bg" in mesh_info_dict, "bg mesh is required"
    bg_mesh = mesh_info_dict["bg"]["mesh"]
...
def _process_mask_exr_files(room, mesh_info_dict, masks_dir, resolution):
    ...
    if args.engine == 'CYCLES':
        ...
        mesh_info_dict_path = os.path.join(os.path.dirname(masks_dir), f"{os.path.basename(masks_dir)}.pkl")
        save_mesh_info_dict = {}
        for mesh_id, mesh_info in mesh_info_dict.items():
            save_mesh_info_dict[mesh_id] = {
                "names": mesh_info["names"],
                "index": mesh_info["index"]
            }
        with open(mesh_info_dict_path, 'wb') as f:
            pickle.dump(save_mesh_info_dict, f, protocol=pickle.HIGHEST_PROTOCOL)
        default_read_mask_script = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "procthor", "read_mask.py")
        )
        cmd = [
            sys.executable,
            os.environ.get("MANSIONWORLD_READ_MASK_SCRIPT", default_read_mask_script),
            "--mask_dir", masks_dir, "--mesh_info_dict_path", mesh_info_dict_path
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("scene_path", type=str, help="Scene path")
    ...
    try:
        ...
        for room_id, room_geoms in scene_geoms.items():
            ...
            output_dir = os.path.join(args.output_dir, room_save_name)

            room_region = room_polygon_to_xy_array(room_polygons_dict[room_id])
            print(f"room_region: {room_region}")
            ...
            env = CameraPlannerEnv(bounds, all_meshes, interest_meshes, room_region=room_region)
