...
class TimestepEmbedder(nn.Module):
    ...
    def __init__(self, hidden_size, frequency_embedding_size=256, max_period=10000):
        ...
        # Pre-compute and cache frequency coefficients as buffer (avoids creating tensors every call)
    ...
    def forward(self, t):
        """
        Create sinusoidal timestep embeddings and pass through MLP.
        ...
        Returns:
            t_emb: an (N, hidden_size) Tensor of timestep embeddings.
        """
        # Use cached frequencies - already on correct device via buffer mechanism
        args = t[:, None].float() * self.freqs[None]  # [N, half]
...
class FlowModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, cond: torch.Tensor, cond_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        assert [*x.shape] == [x.shape[0], self.in_channels, *[self.resolution] * 3], \
                f"Input shape mismatch, got {x.shape}, expected {[x.shape[0], self.in_channels, *[self.resolution] * 3]}"

        input_dtype = x.dtype

        # Reshape input: [B, C, D, H, W] -> [B, D*H*W, C]
        ...
        # Cast to model dtype once at the start (only if not using autocast)
        if not torch.is_autocast_enabled():
            if h.dtype != self.dtype:
                h = h.to(self.dtype)
            if t_emb.dtype != self.dtype:
                t_emb = t_emb.to(self.dtype)
            if cond.dtype != self.dtype:
                cond = cond.to(self.dtype)
            if cond_mask is not None and cond_mask.dtype != self.dtype:
                cond_mask = cond_mask.to(self.dtype)

        for block in self.blocks:
            h = block(h, t_emb, cond, cond_mask, self.rope_phases)

        # Cast back to input dtype once at the end
        if not torch.is_autocast_enabled() and h.dtype != input_dtype:
            h = h.to(input_dtype)
